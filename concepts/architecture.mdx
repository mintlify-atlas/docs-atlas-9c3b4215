...
<Steps>
  ...
  <Step title="CORS">
    `UseCors("AllowAngular")` applies the CORS policy that permits the Angular web portal origins (`localhost:4200`, `app.tramiya.com.py`, `tramiya.com.py`) and any header or method. Running CORS before authentication ensures that preflight `OPTIONS` requests are answered without requiring a valid Bearer token.
  </Step>
  ...
  <Step title="Static Files & Controllers">
    `UseStaticFiles` serves uploaded files from `wwwroot/uploads` directly, after authorization. `MapControllers` then dispatches authenticated, authorized requests to the appropriate controller action.
  </Step>
</Steps>
