...
| `Severity` | `NVARCHAR(20)` | `Info` or `Warning` |
...
Sets `IsVisibleForAdmins = 0`, records `HiddenByUserId` and `HiddenAtUtc`.
...
Sets `IsVisibleForAdmins = 1` and clears `HiddenByUserId` and `HiddenAtUtc`.
